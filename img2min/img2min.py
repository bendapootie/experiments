...
def get_next_best_rect(ground_img, progress_img, rect_size, iterations):
...
def build_rectangle_list_from_image(img, max_num_rects):
    ...
    while (len(out_rects) < max_num_rects):
        ...
        denom = 2.0 * (1 - t) + (img.width / 1.5) * (t)
        s = int(img.width / denom)
        size = (s, s)

        next_rect = get_next_best_rect(img, out_img, size, ITERATION_SCALAR)
